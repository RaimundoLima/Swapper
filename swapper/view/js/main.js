...
function gerarCards(data) {
    ...
    for(var i = 0; i < Object.keys(dataCard).length; i++) {
        ...
        for(var j = 0; j < Object.keys(dataCard[i].roupa).length; j++){
            const roupa = dataCard[i].roupa[j];
            const usuario = dataCard[i].usuario;
        ...
        if (msgsNovas != '') {
            for (var i = Object.keys(msgsNovas).length - 1; i >= 0; i--) {
                ...
                if (msgsNovas[i].usuario == 0) {
                    html += `<div id="" class="msgSystem tx-c">${msgsNovas[i]['conteudo']}</div>`;
                } else if (msgsNovas[i].usuario == 1) {
                    html += `<div class="msgSent tx-r">
                                <div>
                                    <div class="cont tx-l">
                                        <span>${msgsNovas[i]['conteudo']}</span>
                                    </div>
        ...
        for(var i = 0; i < Object.keys(dataChat).length; i++){
            ...
            html += `<div class="combinacoes_msg_card" onclick="buscarMensagens(${dataChat[i].idChat})">
                        <div class="row">
                            <div class="col s3">
                                <div class="pic_msgs">
                                    <img id="${likeStat}" src="../../${dataChat[i].fotoUsuario}" alt="">
                                </div>
                            </div>
                            <div class="row">
...
function buscarMensagens(idChat){
    ...
    $.ajax({
        ...
        for (var i = Object.keys(msgs['msgs']).length - 1; i >= 0; i--) {
            ...
            if (msgs['msgs'][i].usuario == 0) {
                ...
                html += `<div class="msgSent tx-r">
                            <div>
                                <div class="cont tx-l">
                                    <span> ${msgs['msgs'][i]['conteudo']} </span>
                                </div>
                                <i class="vizu material-icons"> done </i>
                                <span class="hora"> ${horario} </span>
                            </div>
        ...
        $("#fotoChat").attr('src', '../../' + msgs['usuario']['foto']);
        ...
        for (var i = Object.keys(msgsNovas.msgs).length - 1; i >= 0; i--) {
            ...
            if (msgsNovas.msgs[i].usuario == 0) {
                ...
                html += `<div class="msgSent tx-r">
                            <div>
                                <div class="cont tx-l">
                                    <span> ${msgsNovas.msgs[i]['conteudo']} </span>
                                </div>
                                <i class="vizu material-icons"> done </i>
                                <span class="hora"> ${horario} </span>
                            </div>
